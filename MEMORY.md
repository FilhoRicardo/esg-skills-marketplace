...
- RF-100 PR #6 was merged to main on 2026-06-23 with the Calm Ledger frontend, site-native submission form, and GitHub workflow-dispatch intake; the here.now site was redeployed automatically.
- RF-100 intake pipeline rewrite is on branch `codex/rf-100-drive-intake` (PR #7, draft). Replaces workflow-dispatch with: here.now Drive private inbox → launchd sync → isolated Codex review → deterministic publisher → SSH deploy key push → `submission-reviewed/**` branch → GitHub opens draft PR. External setup still required before going live (see `docs/INTAKE-SETUP.md`): create Drive, mint write token, update proxy.json Drive ID, obtain CODEX_API_KEY, create SSH deploy key, load launchd agents.
- Administrator branch protection with CODEOWNER review is enforced on main. The sole-maintainer bootstrap exception was used once for PR #6 merge; future merges require a second reviewer or a documented maintenance window.
- `storm-ecm` was added to the approved marketplace catalogue on 2026-06-30 under operations, with its HTML report shell stored as a text template to keep the existing text-only policy intact.
