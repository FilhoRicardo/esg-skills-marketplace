...
- accepts any meaningful UTF-8 Markdown instructions and a public title, then generates title-only intake metadata before dispatch;
...
- The intake workflow writes only the submitted skill folder and preserves its Markdown unchanged. The draft remains blocked until a maintainer adds valid frontmatter, assigns a category, and regenerates the catalogue artefacts.
